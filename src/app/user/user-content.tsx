...
function getLevelColor(level: number) {
  if (level >= 50000) return { text: 'text-red-400', bg: 'bg-red-400' }
  if (level >= 10000) return { text: 'text-yellow-400', bg: 'bg-yellow-400' }
  if (level >= 5000) return { text: 'text-amber-400', bg: 'bg-amber-400' }
  if (level >= 1000) return { text: 'text-cyan-400', bg: 'bg-cyan-400' }
  if (level >= 500) return { text: 'text-purple-400', bg: 'bg-purple-400' }
...
export function UserContent() {
  ...
  const expProgress = expForNextLevel > 0 ? ((userProfile.exp || 0) / expForNextLevel) * 100 : 100
      ...
      <div className="relative px-4 -mt-12 sm:-mt-14">
        ...
        <div className="mt-4 space-y-1.5">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium text-foreground">{userProfile.exp || 0} / {expForNextLevel}</span>
          </div>
